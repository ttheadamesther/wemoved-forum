...
export default function Navbar() {
  ...
  const NavBtn = ({ to, label, icon }) => {
    ...
    return (
      <Link to={to}>
        <button className="nb" style={{
          padding: '0 16px', height: 64, border: 'none', cursor: 'pointer',
          background: active ? 'rgba(200,162,0,.2)' : 'transparent',
  ...
  return (
    <nav style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 999, height: 64, display: 'flex', alignItems: 'center', padding: '0 20px', background: '#111', borderBottom: `2px solid ${C.navBorder}`, gap: 8 }}>
      
      {/* Logo */}
   <Link to="/" style={{ marginRight: 16, flexShrink: 0, marginTop: 8 }}>
  <Logo height={70} />
</Link>

      {/* Nav links */}
      <NavBtn to="/"          label="Accueil"   icon="🏠" />
      <NavBtn to="/forum"     label="Forum"     icon="💬" />
      <NavBtn to="/members"   label="Membres"   icon="👥" />
      {user && <NavBtn to="/messages"    label="Messages privés" icon="✉️" />}
      {user && <NavBtn to="/profile"     label="Mon Profil"      icon="👤" />}
      ...
      <div ref={searchRef} style={{ marginLeft: 'auto', position: 'relative' }}>
        <div style={{ display: 'flex', alignItems: 'center', background: '#222', border: '1px solid #333', borderRadius: 20, padding: '0 12px', gap: 8 }}>
          ...
          <input
            value={search}
            onChange={e => { setSearch(e.target.value); setShowRes(true) }}
            onFocus={() => setShowRes(true)}
            placeholder="Rechercher…"
            style={{ background: 'none', border: 'none', outline: 'none', color: '#fff', fontSize: 13, width: 180, padding: '8px 0', fontFamily: "'Open Sans',sans-serif" }}
          />
        </div>
        {showRes && results.length > 0 && (
          <div style={{ position: 'absolute', top: '110%', right: 0, width: 240, background: '#fff', border: `1px solid ${C.border}`, borderRadius: 4, boxShadow: '0 4px 16px rgba(0,0,0,.15)', overflow: 'hidden', zIndex: 1000 }}>
            {results.map(u => (
              <div key={u.id} onClick={() => { navigate(`/members/${u.id}`); setSearch(''); setShowRes(false) }} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', cursor: 'pointer', borderBottom: `1px solid ${C.border}` }}
      ...
      {user && (
        <div ref={notifRef} style={{ position: 'relative' }}>
          ...
          {showNotifs && (
            <div style={{ position: 'absolute', top: '110%', right: 0, width: 300, background: '#fff', border: `1px solid ${C.border}`, borderRadius: 4, boxShadow: '0 4px 16px rgba(0,0,0,.15)', zIndex: 1000, overflow: 'hidden' }}>
              ...
              {notifs.length === 0
                ...
                : notifs.map(n => (
                  <div key={n.id} onClick={() => { markRead(n.id); if (n.link) navigate(n.link) }} style={{ padding: '10px 14px', borderBottom: `1px solid ${C.border}`, cursor: 'pointer', background: '#fffae6', fontSize: 12, color: C.text }}
                    onMouseEnter={e => e.currentTarget.style.background = '#fff8d0'}
                    onMouseLeave={e => e.currentTarget.style.background = '#fffae6'}
                  >
                    {n.content}
                  </div>
                ))
      ...
      {/* Profil utilisateur */}
      {user ? (
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginLeft: 8, padding: '0 12px', height: 44, background: '#222', borderRadius: 22, border: '1px solid #333' }}>
        ...
        <div style={{ display: 'flex', gap: 8, marginLeft: 8 }}>
          <Link to="/login"><button className="nb" style={{ padding: '0 14px', height: 36, border: '1px solid #333', borderRadius: 3, cursor: 'pointer', background: 'transparent', color: '#ccc', fontSize: 12 }}>Connexion</button></Link>
