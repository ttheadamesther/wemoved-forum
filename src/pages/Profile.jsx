...
export default function Profile() {
  ...
  const uploadPhoto = async (e) => {
    ...
    await patchProfile({ photos: [...(profile.photos || []), url] })
    ...
    <div style={{ maxWidth: 860, margin: '0 auto', padding: '24px 16px' }}>
      ...
      {cropSrc && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.85)', zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ background: C.white, borderRadius: 8, overflow: 'hidden', width: 400, boxShadow: '0 8px 32px rgba(0,0,0,.4)' }}>
            ...
            <div style={{ padding: '0 20px 16px', display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
              <Btn onClick={cancelCrop} variant="ghost">Annuler</Btn>
              <Btn onClick={confirmCrop} variant="yellow">Confirmer</Btn>
            </div>
          </div>
        </div>
      )}
      ...
      <div style={{ background: C.white, border: `1px solid ${C.border}`, borderRadius: 4, padding: 24, marginBottom: 12, display: 'flex', gap: 20, alignItems: 'flex-start' }}>
        ...
        <div style={{ position: 'relative', flexShrink: 0 }}>
          <div onClick={() => avatarRef.current.click()} style={{ width: 80, height: 80, borderRadius: 4, background: '#444', border: '2px solid #333', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: 28, color: '#fff', cursor: 'pointer', overflow: 'hidden', position: 'relative' }}>
            {profile.avatar_url
        ...
        <div style={{ flex: 1 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
            <span style={{ fontWeight: 700, fontSize: 18, color: C.text }}>@{profile.pseudo}</span>
        ...
        <Btn onClick={startEdit} variant="ghost">Modifier</Btn>
      </div>
...
{ label: 'Votes reçus', value: totalVotes },
        ...
        {/* Votes reçus */}
        <div style={{ background: C.white, border: `1px solid ${C.border}`, borderRadius: 4, padding: 16 }}>
          <div style={{ fontWeight: 700, fontSize: 11, color: '#888', textTransform: 'uppercase', letterSpacing: .5, marginBottom: 10 }}>Votes reçus</div>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 10 }}>
            {(profile.interests || []).length === 0
              ...
              : (profile.interests || []).map(i => (
                <span key={i} onClick={() => removeInterest(i)} style={{ padding: '3px 10px', borderRadius: 20, fontSize: 11, background: '#f5f5f5', color: C.textMid, border: '1px solid #ddd', cursor: 'pointer' }} title="Cliquer pour supprimer">
                  {i} ✕
                </span>
              ))
            }
      ...
      <div style={{ background: C.white, border: `1px solid ${C.border}`, borderRadius: 4, padding: 16, marginTop: 12 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
          <div style={{ fontWeight: 700, fontSize: 11, color: '#888', textTransform: 'uppercase', letterSpacing: .5 }}>Photos</div>
        ...
        {(profile.photos || []).length === 0
          ? <div style={{ textAlign: 'center', padding: '20px', color: C.textDim, fontSize: 12, fontStyle: 'italic' }}>Aucune photo ajoutée</div>
          : (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(140px, 1fr))', gap: 8 }}>
              {(profile.photos || []).map((url, i) => (
                <div key={i} style={{ position: 'relative', aspectRatio: '1', borderRadius: 3, overflow: 'hidden', border: `1px solid ${C.border}` }}>
                  <img src={url} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                  <button onClick={() => removePhoto(url)} style={{ position: 'absolute', top: 4, right: 4, width: 22, height: 22, borderRadius: '50%', border: 'none', background: 'rgba(0,0,0,.6)', color: '#fff', cursor: 'pointer', fontSize: 12, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>✕</button>
                </div>
              ))}
            </div>
          )
