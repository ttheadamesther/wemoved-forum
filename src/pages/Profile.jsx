...
function getCroppedBlob(imageSrc, pixelCrop) {
  return new Promise((resolve, reject) => {
    ...
    image.onload = () => {
      ...
      ctx.drawImage(image, Math.round(pixelCrop.x), Math.round(pixelCrop.y), Math.round(pixelCrop.width), Math.round(pixelCrop.height), 0, 0, Math.round(pixelCrop.width), Math.round(pixelCrop.height))
...
function BannerPositionModal({ url, initialPosition, onConfirm, onCancel }) {
  ...
  const moveDrag = (x, y) => {
    ...
    const dx = (x - last.current.x) / rect.width * 100
    const dy = (y - last.current.y) / rect.height * 100
    last.current = { x, y }
    setPos(o => ({ x: Math.min(100, Math.max(0, o.x - dx)), y: Math.min(100, Math.max(0, o.y - dy)) }))
  }
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.85)', zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0 16px' }}>
      <div style={{ background: '#1a1a2e', borderRadius: 14, overflow: 'hidden', width: '100%', maxWidth: 800, boxShadow: '0 8px 40px rgba(0,0,0,.6)' }}>
        ...
        <div ref={containerRef}
          ...
          onTouchEnd={stopDrag}
          style={{ height: 220, overflow: 'hidden', cursor: 'grab', userSelect: 'none' }}>
          <img src={url} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover', objectPosition: posStr, pointerEvents: 'none', display: 'block' }} />
        </div>
        <div style={{ padding: '0 20px 16px', marginTop: 14, display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
...
export default function Profile() {
  ...
  useEffect(() => {
    ...
    const loadFriends = async () => {
      ...
      const friendIds = [...(Array.isArray(d1) ? d1.map(f => f.user_b) : []), ...(Array.isArray(d2) ? d2.map(f => f.user_a) : [])]
    ...
    const loadPending = async () => {
      ...
      } else { setPendingRequests([]); setPendingProfiles([]) }
    }
  ...
  const acceptFriend = async (friendship) => {
    const token = await getToken()
    await fetch(`${SUPABASE_URL}/rest/v1/friendships?id=eq.${friendship.id}`, { method: 'PATCH', headers: { 'apikey': ANON_KEY, 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ status: 'accepted' }) })
  ...
  const rejectFriend = async (friendship) => {
    const token = await getToken()
    await fetch(`${SUPABASE_URL}/rest/v1/friendships?id=eq.${friendship.id}`, { method: 'DELETE', headers: { 'apikey': ANON_KEY, 'Authorization': `Bearer ${token}` } })
    setPendingRequests(p => p.filter(f => f.id !== friendship.id))
  ...
  const saveBannerPosition = async (pos) => { await patchProfile({ banner_position: pos }) }
  ...
  const selectBanner = async (e) => {
    ...
    const token = await getToken(); const ts = Date.now()
    ...
    setUploadingBanner(false); setShowPositionSlider(true); e.target.value = ''
  }
  ...
  return (
    <div style={{ maxWidth: 1200, margin: '0 auto', paddingBottom: 80 }}>
      ...
      {lightbox && (() => {
        ...
        const goTo = (idx) => { const newIdx = (idx + total) % total; openLightbox(photos[newIdx], newIdx) }
                    ...
                    return (
                      <div key={lk.id} style={{ display: 'flex', alignItems: 'center', gap: 5, background: 'rgba(255,255,255,.1)', borderRadius: 20, padding: '3px 10px' }}>
                        <div style={{ width: 20, height: 20, borderRadius: '50%', background: lk.avatar_url ? '#444' : lkColor, overflow: 'hidden', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 8, fontWeight: 700, color: '#fff' }}>
                          {lk.avatar_url ? <img loading="lazy" src={lk.avatar_url} style={{ width: '100%', height: '100%', objectFit: 'cover' }} alt="" /> : lk.initials || lk.pseudo?.slice(0,2).toUpperCase()}
                        </div>
      ...
      <div style={{ background: 'var(--white)', border: '1px solid var(--border)', borderRadius: 16, overflow: 'hidden', boxShadow: '0 2px 16px rgba(0,0,0,.1)' }}>
        ...
        <div style={{ height: 200, overflow: 'hidden', position: 'relative', background: profile.banner_url ? 'transparent' : (profile.banner_gradient || BANNER_GRADIENTS[0]), flexShrink: 0 }}>
          {profile.banner_url && (
            <img src={profile.banner_url} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover', objectPosition: bannerPos, display: 'block' }} />
          )}
          {/* Boutons bannière */}
          ...
          {showBannerPicker && (
            <div style={{ position: 'absolute', top: 48, right: 12, background: C.white, borderRadius: 12, padding: 14, boxShadow: '0 8px 32px rgba(0,0,0,.3)', zIndex: 200, width: 260 }}>
        ...
        <div style={{ padding: '0 20px 20px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginTop: -44 }}>
            <div style={{ position: 'relative' }}>
              <div onClick={() => avatarRef.current.click()} style={{ width: 88, height: 88, borderRadius: '50%', background: profile.avatar_url ? '#444' : avatarColor, border: ROLE_RING[profile.role] ? `4px solid ${ROLE_RING[profile.role]}` : '4px solid var(--white)', boxShadow: ROLE_RING[profile.role] ? `0 0 16px ${ROLE_RING[profile.role]}99` : '0 4px 16px rgba(0,0,0,.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: 28, color: '#fff', cursor: 'pointer', overflow: 'hidden', position: 'relative', zIndex: 2 }}>
          ...
          <div style={{ marginTop: 14 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10, flexWrap: 'wrap' }}>
              <span style={{ fontWeight: 700, fontSize: 20, color: 'var(--text)' }}>@{profile.pseudo}</span>
      ...
      <div style={{ padding: '0 16px', display: 'flex', flexDirection: 'column', gap: 12 }}>
        <div style={{ display: 'grid', gridTemplateColumns: isDesktop ? '1fr 340px' : '1fr', gap: 16, alignItems: 'start' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            <div style={PANEL}>
              ...
              {editing ? (
                ...
                <div style={{ fontSize: 13, color: profile.bio ? C.text : C.textDim, lineHeight: 1.7, fontStyle: profile.bio ? 'normal' : 'italic', whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>
              ...
              <div style={{ ...PANEL, borderTop: '3px solid #e67e22' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  {pendingRequests.map(friendship => {
                    ...
                    const fColor = colors[(sender.pseudo?.charCodeAt(0) || 0) % colors.length]
                    return (
                      <div key={friendship.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 14px', background: C.accentBg, border: `1px solid ${C.accentDk}`, borderRadius: 12 }}>
                        <div style={{ width: 40, height: 40, borderRadius: '50%', background: sender.avatar_url ? '#444' : fColor, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 700, color: '#fff', overflow: 'hidden', flexShrink: 0 }}>
                          {sender.avatar_url ? <img loading="lazy" src={sender.avatar_url} style={{ width: '100%', height: '100%', objectFit: 'cover' }} alt="" /> : sender.initials || sender.pseudo?.slice(0,2).toUpperCase()}
                        </div>
                        <div style={{ flex: 1 }}>
