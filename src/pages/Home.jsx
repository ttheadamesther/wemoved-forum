...
export default function Home() {
  ...
  return (
    <div ref={containerRef} style={{ maxWidth: 1280, margin: '0 auto', padding: isMobile ? '16px 12px' : '24px 16px', ...gridStyle }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <h2 style={{ fontWeight: 700, fontSize: isMobile ? 14 : 16, color: '#ddd', textTransform: 'uppercase', letterSpacing: 1 }}>Discussions récentes</h2>
          {user && (
            <button onClick={() => setNewThread(t => !t)} style={{ padding: isMobile ? '7px 12px' : '8px 16px', background: `linear-gradient(to bottom,#f0c800,#c8a200)`, border: 'none', borderRadius: 4, cursor: 'pointer', fontWeight: 700, fontSize: isMobile ? 12 : 13, color: '#3a2e00' }}>
              + Nouvelle discussion
            </button>
          )}
        </div>
        ...
        {newThread && (
          <div style={{ background: C.white, border: `1px solid ${C.border}`, borderRadius: 6, padding: 16 }}>
            ...
            <textarea value={body} onChange={e => setBody(e.target.value)} placeholder="Contenu…" rows={3} style={{ width: '100%', padding: '8px 10px', borderRadius: 3, border: `1px solid ${C.borderMid}`, fontSize: 13, color: C.text, fontFamily: "'Open Sans',sans-serif", resize: 'vertical', marginBottom: 10, boxSizing: 'border-box' }} />
        ...
        <div style={{ background: C.white, border: `1px solid ${C.border}`, borderRadius: 6, overflow: 'hidden' }}>
          ...
          {threads.map(t => {
            ...
            return (
              <div key={t.id} onClick={() => navigate('/forum')} style={{ display: 'flex', alignItems: 'center', gap: isMobile ? 8 : 12, padding: isMobile ? '12px' : '14px 16px', borderBottom: `1px solid ${C.border}`, cursor: 'pointer' }}
                onMouseEnter={e => e.currentTarget.style.background = '#fafafa'}
                onMouseLeave={e => e.currentTarget.style.background = '#fff'}
              >
                {!isMobile && (
                ...
                <div style={{ width: 28, height: 28, borderRadius: 3, background: '#444', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 700, color: '#fff', overflow: 'hidden', flexShrink: 0 }}>
        ...
        {/* Citation */}
        <div style={{ background: '#1a1a1a', borderRadius: 6, padding: isMobile ? '20px 20px' : '24px 32px', textAlign: 'center' }}>
          <div style={{ fontSize: 32, color: C.accent, lineHeight: 1, marginBottom: 8 }}>"</div>
          <p style={{ fontSize: isMobile ? 14 : 16, color: '#fff', fontWeight: 700, marginBottom: 6 }}>La communauté, c'est ce qui nous fait avancer.</p>
          ...
          <div style={{ background: C.white, border: `1px solid ${C.border}`, borderRadius: 6, padding: 16 }}>
            ...
            <div style={{ display: 'flex', gap: 8, marginBottom: 14, flexWrap: 'wrap' }}>
              {BADGES_DEF.map(b => {
                ...
                return (
                  <div key={b.key} title={b.label} style={{ width: 38, height: 38, borderRadius: '50%', background: has ? '#fffae6' : '#f5f5f5', border: `2px solid ${has ? C.accentDk : '#ddd'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, opacity: has ? 1 : 0.35 }}>
                    {b.emoji}
                  </div>
                )
              })}
